...
function ProductList({ onHomeClick }) {
    ...
    // Task 4: Calculate total quantity for the cart icon badge
    ...
    const plantsArray = [
        ...
        },
        {
            ...
            plants: [
                { name: "Oregano", image: "https://cdn.pixabay.com/photo/2015/05/30/21/20/oregano-790702_1280.jpg", description: "Deters certain insects with its natural compounds.", cost: "$10" },
                ...
                { name: "Geraniums", image: "https://cdn.pixabay.com/photo/2012/04/26/21/51/flowerpot-43270_1280.jpg", description: "Known for insect-repelling properties and a pleasant scent.", cost: "$20" },
                    ...
                    <div> 
                        <a href="#" onClick={(e) => handleCartClick(e)} style={styleA}>
                            <h1 className='cart'>
                                ...
                                {totalQuantity > 0 && <span style={{ position: 'absolute', top: '10px', right: '10px', background: 'black', borderRadius: '50%', padding: '5px 10px', fontSize: '20px' }}>{totalQuantity}</span>}
                            </h1>
                        </a>
                    </div>
                </div>
                        ...
                        <div key={index}>
                            ...
                            <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'center' }}>
                                {category.plants.map((plant, idx) => (
                                    <div className="product-card" key={idx} style={{ border: '1px solid #ccc', margin: '15px', padding: '15px', borderRadius: '10px', width: '350px', textAlign: 'center' }}>
                                        <img src={plant.image} alt={plant.name} style={{ width: '100%', height: '250px', objectFit: 'cover', borderRadius: '8px' }} />
                                        <h2 style={{ margin: '10px 0' }}>{plant.name}</h2>
                                        <p>{plant.description}</p>
                                        <p style={{ fontWeight: 'bold', fontSize: '20px' }}>{plant.cost}</p>
                                        <button 
                                            disabled={cart.some(item => item.name === plant.name)}
                                            onClick={() => handleAddToCart(plant)}
                                            style={{ backgroundColor: cart.some(item => item.name === plant.name) ? '#808080' : '#4CAF50', color: 'white', padding: '10px 20px', border: 'none', borderRadius: '5px', cursor: 'pointer', width: '100%' }}
                                        >
